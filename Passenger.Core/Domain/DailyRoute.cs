...
namespace Passenger.Core.Domain
{
    public class DailyRoute
    {
        ...
        public void AddPassaengerNode(Passenger passenger, Node node)
        {
            var passengerNode = GetPassengerNode(passenger);
            if (passengerNode != null)
            {
                throw new InvalidOperationException($"Node already exists for passenger {passengerNode.Passenger.UserId}");
            }
